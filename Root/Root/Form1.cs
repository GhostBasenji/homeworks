...
// функции TryParse. Первым параметром метода TryParse является анализируемое поле TextBox1.Text.
// Второй параметр — это разрешаемый для преобразования стиль числа, он может быть целого типа (Integer), шестнадцатеричным (HexNumber), представленным
// в экспоненциальном виде, и проч.
// Третий параметр указывает, на какой основе формируется допустимый формат, — в нашем случае мы использовали CurrentInfo, т. е. на основе текущего языка и региональных параметров.
...
// Как видно из текста программы, если пользователь ввел не число (например, какие-либо буквы), то на метку Label1 выводится красным цветом текст "Следует вводить числа".
...
// Если пользователь ввел все-таки число, то будет выполняться следующий оператор извлечения квадратного корня Math.Sqrt(X). Функция Math.Sqrt(X)возвращает значение
// типа double (двойной точности с плавающей запятой), которое приводим с помощью преобразования CType или Convert.ToSingle к переменной одинарной точности типа Single.
...
// Использованный формат "Корень из {0} равен {1:F5}" означает — взять нулевой выводимый элемент, т. е. переменную X, и записать эту переменную вместо фигурных скобок, после чего взять 
